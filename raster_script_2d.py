...
class TIME_TELE :
    ...
    def pos_update(self,queue2,sec,map_size,map_angle,coord,epoch,object,map_len,num_scans):
        ...
        num_loop = int(float(map_len) // (0.43/60.0) + (float(map_len) % (0.43/60.0) > 0))
        i = 0
        while not ut.tel_exit.is_set() :
            ...
            if i <= num_loop:
                ...
                if i == 0 :
                    ...
                    ''' Need to find out how to convert to correct coordintaes'''
                    msg = 'TIME_SEEK %s %s %s %s' %(coord[0]+(i*0.43),coord[1]+(i*0.43),epoch,object)
                    self.s.send(msg)
                    reply = self.s.recv(1024).decode("ascii")
                    if 'OK' in reply  :
                        continue
