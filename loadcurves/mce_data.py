# This file is provided by UBC in MAS, but has been modified to support Python 3
...
class BitField(object):
	...
	def extract(self, data, rescale=True, unwrap=False, **kwargs):
		...
		if self.signed:
			...
			data = (data.astype(int) >> self.start) & ((1 << self.count)-1)
		if unwrap:
...
class SmallMCEFile:
	...
	def _GetContentInfo(self):
		...
		if self.data_mode is None:
			raise ValueError("Could not determine data_mode.  Is the <name>.run file missing?")
