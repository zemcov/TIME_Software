...
if __name__ == '__main__' :
    ...
    rabins = np.linspace(75.5,76.7,2161)
    decbins = np.linspace(23.8,24.8,1801)
    ramesh, decmesh = np.meshgrid(rabins,decbins,indexing='ij')
    
    # symmetric Gaussian, source centre located slightly above scan centre
    fakecube = np.exp(-((ramesh-76.1)**2+(decmesh-24.6)**2)[...,None]/0.001764/(np.arange(60)[None,None,:]+1)**0.25)
    ...
    fakecube_det = cube_to_ts(faketime_det,faketime,fakera,fakedec,fakecube,
                                rabins,decbins,noise=0.2*(np.arange(60)+1)**0.5)
    ...
    # pl.ylabel('dec')
    # pl.subplot(122)
    # pl.pcolormesh(range(60),mapdec,np.ma.masked_invalid(maps[:,9].T),
    #                 vmin=-0.3,vmax=0.9)
    # pl.xlabel('channel')
    # pl.gca().set_yticks(())
    # pl.show()
