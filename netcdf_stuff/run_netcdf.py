...
def netcdfdata():
    ...
    n_files = 8
    filestarttime = 0
    mcea = subprocess.Popen(['ssh -T pilot2@timemce.rit.edu python /home/pilot2/ssh_stuff/mce1_sftp.py %s %s' % (a, n_files)], shell=True)
    while True:
        ...
        if os.path.exists('tempfiles/stop.txt'):
            subprocess.call(["ssh -T pilot2@timemce.rit.edu /usr/mce/bin/mce_cmd '-x stop rcs ret_dat'"], shell=True)
            mcea.terminate()
            subprocess.call(['ssh -T pilot2@timemce.rit.edu "rm /data/cryo/current_data/temp.*"'], shell=True)
...
def readdata(f, mce_file_name, mce, head, n, a, filestarttime):
    ...
    old_mce_file_name = "/home/pilot1/netcdf_stuff/mce1/temp.%0.3i" %(a - 2)
