...
while end - begin < dt.timedelta(seconds = 3):
    if os.path.exists(dir + 'temp.run'):
        if a == 0:
            subprocess.Popen(['scp', dir + 'temp.run',  'time@time-master:/home/time/Desktop/time-data/mce1/temp.run']).wait()
        ...
        if os.path.exists(dir + "temp.%0.3i" %(a+1)) : #wait to read new file until old file is complete
            ...
            if os.path.exists(mce_file_name) :
                subprocess.Popen(['scp', mce_file_name,  'time@time-master:/home/time/Desktop/time-data/mce1/temp.%0.3i' % (a)]).wait()
